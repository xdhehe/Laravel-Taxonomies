...
trait HasTaxonomies
{
	...
	public function addTerm($terms, $taxonomy, $parent = 0, $order = 0) {
        ...
        if (count($terms) > 0) {
            foreach ($terms as $term) {
                if ($this->taxonomies()->where('taxonomy', $taxonomy)->where('term_id', $term)->first())
                    continue;
                $tax = Taxonomy::where('term_id', $term)->where('taxonomy', $taxonomy)->first();
                $this->taxonomies()->attach($tax->id);
            }
            return;
        }
	 ...
	 * @return mixed
	 */
	public function getTerms($taxonomy = '',$parent = -1)
	{
		if (($taxonomy) && ($parent!=-1)) {
			$term_ids = $this->taxonomies->where('taxonomy', $taxonomy)->where('parent', $parent)->pluck('term_id');
		} 
		else if ($taxonomy){
	...
	public function getParentName($parentId){
		$name = DB::table('taxonomies')
		    ->rightJoin('terms', 'taxonomies.parent', '=', 'terms.id')
	...
	public function getChildren($taxonomy='',$parent=-1){
		if ($parent!= -1){
        	$child = DB::table('taxonomies')
                 ->rightJoin('terms', 'taxonomies.term_id', '=', 'terms.id')
